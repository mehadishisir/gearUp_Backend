...
const createPaymentSession = async (rentalOrderId: string, customerId: string) => {
  ...
  if (order.customerId !== customerId) {
    const error: any = new Error("You are not authorized to pay for this order");
    error.statusCode = 403;
    throw error;
  }

  if (order.status !== "CONFIRMED") {
    const error: any = new Error("Order must be confirmed before payment");
  ...
  if (existingPayment) {
    const error: any = new Error("Payment already exists for this order");
...
const getPaymentByIdFromDb = async (id: string, customerId: string) => {
