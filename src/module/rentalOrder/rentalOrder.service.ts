...
import { ICreateRentalOrder } from "./rentalOrder.interface";
...
const createRentalOrderIntoDb = async (payload: ICreateRentalOrder, customerId: string) => {
  ...
  if (!items || items.length === 0) {
    const error: any = new Error("At least one gear item is required");
  ...
  const gearItems = await Promise.all(
    items.map(async (item) => {
      ...
      if (!gear) {
        const error: any = new Error(`Gear item not found: ${item.gearItemId}`);
        error.statusCode = 404;
        throw error;
      }

      if (!gear.available || gear.stock < item.quantity) {
        const error: any = new Error(`Insufficient stock for ${gear.name}`);
        error.statusCode = 400;
        throw error;
      }
...
const getOrderByIdFromDb = async (id: string, userId: string) => {
